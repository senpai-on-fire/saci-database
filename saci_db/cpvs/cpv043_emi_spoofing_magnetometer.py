...
class EMISpoofingMagnetometerCPV(CPV):
    ...
    def __init__(self):
        super().__init__(
            ...
            associated_files=[],
            reference_urls=["https://ieeexplore.ieee.org/document/9245834"],
        )

    def in_goal_state(self, state: GlobalState):
        # Define the goal state, such as orientation estimation error or navigation disruption
        return state.has_property("OrientationEstimationError", True) or state.has_property(
            "NavigationDisruption", True
        )
