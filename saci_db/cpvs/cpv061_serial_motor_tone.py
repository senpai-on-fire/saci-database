...
from saci.modeling.device import (Controller, ESC, Motor, Serial, PWMChannel, ESC, Telemetry)
...
class SerialToneCPV(CPV):
    ...
    def __init__(self):
        ...
        super().__init__(
            ...
            attack_vectors = [BaseAttackVector(name='Serial Command Injection',
                                               signal=SerialAttackSignal(src=ExternalInput(), dst=Serial(), data='ASCII numbers 1-5'),
                                               required_access_level='Physical',
                                               ),
                              BaseAttackVector(name='Serial Command Injection',
                                               signal=SerialAttackSignal(src=ExternalInput(), dst=Serial(), data='PgUp key'),
                                               required_access_level='Physical',
                                               )],
            attack_requirements = ['Computer', 'USB-C cable'],
            attack_impacts = [BaseAttackImpact(category='Loss of Productivity and Revenue',
