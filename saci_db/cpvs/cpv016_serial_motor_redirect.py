...
class SerialRedirectCPV(CPV):
    ...
    def __init__(self):
        super().__init__(
            ...
            attack_vectors=[
                BaseAttackVector(
                    name="Serial DSHOT_CMD_SPIN_DIRECTION_1 Command Injection",
                    signal=SerialAttackSignal(
                        src=ExternalInput(), dst=Serial(), data="7"
                    ),
                    configuration={"repetition": "6"},
                    required_access_level="Physical",
                ),
                BaseAttackVector(
                    name="Serial DSHOT_CMD_SPIN_DIRECTION_2 Command Injection",
                    signal=SerialAttackSignal(
                        src=ExternalInput(), dst=Serial(), data="8"
                    ),
                    configuration={"repetition": "6"},
                    required_access_level="Physical",
                ),
                BaseAttackVector(
                    name="Serial CMD_SAVE_SETTINGS Command Injection",
                    signal=SerialAttackSignal(
                        src=ExternalInput(), dst=Serial(), data="12"
                    ),
                    configuration={"repetition": "6"},
                    required_access_level="Physical",
                ),
